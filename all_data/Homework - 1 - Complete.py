...
def datasetMenu(choice):
    ...
    training_data = 'train_'+ dataset_dict[dataset_choice]+'.csv' 
    testing_data = 'test_'+ dataset_dict[dataset_choice]+'.csv' 
    validation_data = 'valid_'+ dataset_dict[dataset_choice]+'.csv' 
...
def Information_Gain_Entropy(data,column_attribute,target_attribute):
    ...
    dataset_entropy = entropy(data[target_attribute])
    ...
    individual_entropy = np.sum([(counts[i]/np.sum(counts))*entropy(data.where(data[column_attribute]==values[i]).dropna()[target_attribute]) for i in range(len(values))])
...
def Information_Gain_Variance(data,column_attribute,target_attribute):
    ...
    # Calculation of Entropy of the Class Column
    dataset_variance = variance(data[target_attribute])

    # Counts and values of the Column Attribute in play
    values,counts= np.unique(data[column_attribute],return_counts=True)
    
    # Summation of the Entropies of the unique values in the Coljumn Attribute
    individual_variance = np.sum([(counts[i]/np.sum(counts))*variance(data.where(data[column_attribute]==values[i]).dropna()[target_attribute]) for i in range(len(values))])
